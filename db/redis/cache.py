"""
Redis cache layer.

Keys:
  geotags:{url_hash}      — full JSON tag set, TTL 7 days
  geocode:{place_name}    — individual geocode result, permanent
"""

import json
import logging
import os
from typing import Optional

from dotenv import load_dotenv

load_dotenv()

logger = logging.getLogger(__name__)

GEOTAG_TTL = 60 * 60 * 24 * 7  # 7 days

_redis = None
_is_upstash = False


def get_redis():
    """Lazy-initialise Redis (Upstash REST or standard Redis)."""
    global _redis, _is_upstash

    if _redis is not None:
        return _redis

    # ── Prefer Upstash ─────────────────────────────
    upstash_url = os.getenv("UPSTASH_REDIS_REST_URL")
    upstash_token = os.getenv("UPSTASH_REDIS_REST_TOKEN")

    if upstash_url and upstash_token:
        from upstash_redis import Redis
        _redis = Redis(url=upstash_url, token=upstash_token)
        _is_upstash = True
        logger.info("Using Upstash Redis (REST)")
        return _redis

    # ── Fallback: standard Redis ───────────────────
    redis_url = os.getenv("REDIS_URL")
    if redis_url:
        import redis.asyncio as aioredis
        _redis = aioredis.from_url(redis_url, decode_responses=True)
        _is_upstash = False
        logger.info("Using standard Redis (TCP)")
        return _redis

    logger.warning("No Redis configured")
    return None


async def close_redis():
    """Close Redis if using async client."""
    global _redis, _is_upstash
    if _redis is not None and not _is_upstash:
        await _redis.aclose()
    _redis = None


# ── Helper wrappers (handle async vs sync) ────────────────────────

async def _get(r, key):
    if _is_upstash:
        return r.get(key)
    return await r.get(key)


async def _set(r, key, value, ex=None):
    if _is_upstash:
        return r.set(key, value, ex=ex)
    return await r.set(key, value, ex=ex)


# ── Geotag cache ────────────────────────────────────────────────

async def get_cached_geotags(url_hash: str) -> Optional[list[dict]]:
    r = get_redis()
    if r is None:
        return None

    raw = await _get(r, f"geotags:{url_hash}")
    if raw is None:
        return None

    logger.info("Cache HIT for geotags:%s", url_hash)
    return json.loads(raw)


async def set_cached_geotags(url_hash: str, tags: list[dict]) -> None:
    r = get_redis()
    if r is None:
        return

    await _set(r, f"geotags:{url_hash}", json.dumps(tags), ex=GEOTAG_TTL)
    logger.info("Cached %d geotags for %s", len(tags), url_hash)


# ── Geocode cache ───────────────────────────────────────────────

async def get_cached_geocode(place_name: str) -> Optional[tuple[float, float]]:
    r = get_redis()
    if r is None:
        return None

    raw = await _get(r, f"geocode:{place_name}")
    if raw is None:
        return None

    data = json.loads(raw)
    if data is None:
        return None

    return (data["lat"], data["lng"])


async def set_cached_geocode(place_name: str, lat: float, lng: float) -> None:
    r = get_redis()
    if r is None:
        return

    await _set(
        r,
        f"geocode:{place_name}",
        json.dumps({"lat": lat, "lng": lng}),
    )


async def set_cached_geocode_miss(place_name: str) -> None:
    r = get_redis()
    if r is None:
        return

    await _set(r, f"geocode:{place_name}", json.dumps(None))